        ...
        if ($insertDetails) {
            ...
            if (mysqli_num_rows($getDetails) > 0) {
                while ($saleData = mysqli_fetch_array($getDetails)) {
                    ...
                    $productDesc = $saleData['productDescription'];
                    ...
                    $insertSale = mysqli_query($con, "INSERT INTO sales(invoiceId, productId, productName, quantity, price, subtotal, customerId, userId, dateCreated) 
                    VALUES('{$invoiceId}', '{$productId}', '{$productName}', '{$quantity}', '{$price}', '{$subTotal}', '{$customerId}', '{$userId}', '{$now}')");        
                }
